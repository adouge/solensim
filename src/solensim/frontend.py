...
class Tracker(wrapper.TrackHandle):
    ...
    def check_felddurchgang(self, label=None, title=""):
        ...
        plt.plot(heads.get("z").values, heads.get("pr").values*1/heads["pr"].abs().max(), "-r", label="Avg. radial momentum [max($p_{r}$)]")
        plt.plot(heads.get("z").values, heads.get("pphi").values*1/heads["pr"].abs().max(), "-b", label="Avg. rot. momentum [max($p_{r}$)]")
        plt.plot(heads.get("z").values, heads.get("turn").values, "-g", label="Avg. Larmor angle [$\pi\cdot$rad]")
        z, Bz = self.astra.read_field()
        plt.plot(z+z_solenoid, Bz/np.max(Bz), "--k", label="On-axis field [max($B_z$)]")
        plt.xlabel("Axial position [m]", fontsize=32)
